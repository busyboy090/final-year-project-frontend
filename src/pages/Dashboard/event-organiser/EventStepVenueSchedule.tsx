...
export default function EventStepVenueSchedule({
  register,
  control,
  errors,
  getValues,
  ...
  const venues = Array.isArray(responseData)
    ? responseData
    : responseData?.items || [];
        ...
        <div className="col-span-12 lg:col-span-8 space-y-8">
            ...
            {isLoading ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-96 items-center justify-center text-center bg-white rounded-xl border border-dashed">
                <p className="text-slate-400 text-sm col-span-2">
                  Syncing premium structural arrays...
                </p>
              </div>
            ) : venues.length === 0 ? (
              <div className="grid grid-cols-1 h-80 items-center justify-center text-center bg-white rounded-xl border border-dashed p-6">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-slate-400 max-w-sm mx-auto">
                    Try loosening your target head-count filters or shifting
                    keyword characters.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Controller
                  ...
                  rules={{
                    required:
                      "Please specify a target campus location room asset",
                  }}
                  render={({ field }) => (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {venues.map((venue: any) => {
                        ...
                        return (
                          <div
                            key={venueId} // Using venueId ensures a primitive, stable key string
                            ...
                            <div className="h-44 relative overflow-hidden bg-slate-50 flex items-center justify-center border-b border-slate-100">
                              {venue.thumbnail ? (
                                ...
                                <div className="flex flex-col items-center justify-center text-slate-300 select-none space-y-1">
                                  ...
                                  <span className="text-[10px] font-medium uppercase tracking-widest text-slate-400/70">
                                    No Media Assets
                                  </span>
                                </div>
                              )}
                            ...
                            <div className="p-5 flex flex-col grow justify-between">
                              <div>
                                <div className="flex justify-between items-start mb-1.5">
                                  <h3 className="text-lg font-bold text-[#001e40] leading-snug">
                                    {venue.name || venue.label}
                                  </h3>
                                  <ShieldCheck
                                    className={`w-4 h-4 shrink-0 mt-1 ${isSelected ? "text-[#7b5800]" : "text-slate-400"}`}
                                  />
                                </div>
                                <p className="text-xs text-slate-500 flex items-center gap-1 mb-2">
                                  <MapPin className="w-3 h-3 text-slate-400" />{" "}
                                  {venue.location || "Main Campus"}
                                </p>
                                <p className="text-xs text-slate-400 line-clamp-2 mb-4">
                                  {venue.description ||
                                    "Premium academic deployment target."}
                                </p>
                              </div>
                                ...
                                <div className="flex gap-4 border-t pt-3 text-slate-500">
                                  <div className="flex items-center gap-1">
                                    ...
                                    <span className="text-xs font-semibold">
                                      {venue.capacity || "250"} Cap.
                                    </span>
                                  </div>
                                </div>

                                {/* Changed component from Button to normal div to stop nested interactive bubble click handler collision loops inside Radix */}
                                <div
                                  className={`w-full py-2.5 rounded-lg text-xs font-bold text-center transition-all duration-200 ${
                                    isSelected
                                      ? "bg-[#7b5800] text-white"
                                      : "border border-slate-200 text-[#001e40] group-hover:bg-slate-50"
                                  }`}
                                >
                                  {isSelected
                                    ? "Asset Confirmed"
          ...
          <div className="bg-white p-6 lg:p-8 rounded-xl border shadow-sm space-y-6">
            <h3 className="text-xl font-bold text-[#001e40] flex items-center gap-2">
              <MapPin className="text-[#7b5800]" /> Venue Requirements
            </h3>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <Label className="text-xs uppercase tracking-widest font-semibold text-slate-500">
                  Expected Attendance
                </Label>
                <div className="relative">
                  <Input
                    type="number"
                    {...register("capacity", {
                      // Standardized key assignment registration update
                      required: "Attendance head count estimation is mandatory",
                      min: {
                        value: 5,
                        message: "Events require at least 5 expected attendees",
                      },
                      validate: (value: string) =>
                        Number.isInteger(Number(value)) ||
                        "Attendance must be a whole number",
                    })}
          ...
          <div className="bg-white p-6 lg:p-8 rounded-xl border shadow-sm space-y-6">
            <h3 className="text-xl font-bold text-[#001e40] flex items-center gap-2">
              <Calendar className="text-[#7b5800]" /> Temporal Windows
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div>
                    ...
                    {...register("endDate", {
                      ...
                      validate: (value: any) => {
                        ...
                        return (
                          ...
                          "End Date cannot precede the selected launch date"
                        );
                      },
                    })}
                    className="bg-slate-50 py-6 border-b-2 focus-visible:ring-0"
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Input
                    type="time"
                    {...register("startTime", {
                      required: "Target start configuration hour required",
                    })}
                    className="bg-slate-50 py-6 border-b-2 focus-visible:ring-0"
                  />
                  {errors.startTime && (
                ...
                <div>
                  ...
                  <Input
                    type="time"
                    {...register("endTime", {
                      required: "Wrap-up time required",
                    })}
                    className="bg-slate-50 py-6 border-b-2 focus-visible:ring-0"
                  />
                  {errors.endTime && (
              ...
              <div className="space-y-1">
                ...
                <p className="font-semibold text-white text-base truncate">
                  {selectedVenueDetails
                    ? selectedVenueDetails.name || selectedVenueDetails.label
                    : "None Selected"}
                </p>
              </div>
